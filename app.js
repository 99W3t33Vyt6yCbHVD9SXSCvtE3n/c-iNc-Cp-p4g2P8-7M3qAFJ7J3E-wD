...
(function () {
  ...
  function renderPhoto(step, instant) {
    ...
    if (instant) {
      ...
      // IMPORTANTE: limpiar TODOS los estilos en línea que se acaban de
      // fijar (no solo "transition"). Un estilo en línea (aunque sea
      // opacity:1 u opacity:0) gana siempre a cualquier regla CSS de
      // clase — si no se limpia, esta misma capa queda "atascada" para
      // siempre en esa opacidad/transform, aunque más adelante le toque
      // una foto AUTOMÁTICA con su propia clase de transición. Como solo
      // hay dos capas alternándose, esto dejaba a una de cada dos fotos
      // sin aparecer nunca más tras el primer cambio forzado.
