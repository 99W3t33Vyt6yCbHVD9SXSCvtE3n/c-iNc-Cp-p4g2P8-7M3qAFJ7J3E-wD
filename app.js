...
(function () {
  ...
  // -------------------------------------------------------------------------
  // Motor de transiciones V2
  // -------------------------------------------------------------------------
  // La transición se elige en cada reproducción automática, no al construir
  // la secuencia. Un avance/retroceso forzado nunca usa transición.
  // El fondo usa siempre la misma foto principal, desenfocada, pero cambia
  // después de que termine la entrada de la foto y con un pequeño retraso.
  var TRANSITION_DEFINITIONS = [
    { name: "fade",        enter: "t-fade",        duration: 1200 },
    { name: "zoom-in",     enter: "t-zoom-in",     duration: 1500 },
    { name: "zoom-out",    enter: "t-zoom-out",    duration: 1500 },
    { name: "zoom-soft",   enter: "t-zoom-soft",   duration: 2000 },
    { name: "pan-left",    enter: "t-pan-left",    duration: 1500 },
    { name: "pan-right",   enter: "t-pan-right",   duration: 1500 },
    { name: "pan-up",      enter: "t-pan-up",      duration: 1500 },
    { name: "pan-down",    enter: "t-pan-down",    duration: 1500 },
    { name: "slide-left",  enter: "t-slide-left",  duration: 1100 },
    { name: "slide-right", enter: "t-slide-right", duration: 1100 },
    { name: "slide-up",    enter: "t-slide-up",    duration: 1100 },
    { name: "slide-down",  enter: "t-slide-down",  duration: 1100 },
    { name: "blur",        enter: "t-blur",        duration: 1500 },
    { name: "scale-fade",  enter: "t-scale-fade",  duration: 1400 },
    { name: "kenburns",    enter: "t-kenburns",    duration: 2400 }
  ];
  ...
  // -- Precarga de fotos ---------------------------------------------------
  // Se precarga TODO el capítulo activo. Los capítulos tienen menos de 30
  // fotos, por lo que esto evita que una transición dependa de que la foto
  // siguiente termine de descargarse justo cuando debe entrar.
  var preloadedSrcs = {};
  ...
  function preloadUpcoming(fromIndex, count) {
    // Mantener esta función para el resto del motor: además de precargar
    // el capítulo actual, deja preparadas las primeras fotos siguientes.
    var found = 0;
  ...
  // El primer capítulo queda precargado desde el arranque.
  preloadChapter(0);
  preloadUpcoming(0, 3);
  ...
  // Token de navegación: invalida cualquier temporizador de fondo pendiente
  // cuando el usuario fuerza un cambio o se entra en otra pantalla.
  ...
  function clearPhotoAnimation(layer) {
    if (!layer) return;
  ...
  function restorePhotoAnimationStyles() {
  ...
  function renderStep(instant) {
    ...
    // Cualquier cambio de pantalla invalida un fondo pendiente.
    // Las transiciones visuales ya iniciadas solo se interrumpen si el
    // cambio es forzado; el avance automático mantiene su crossfade.
    if (instant) {
      invalidateTransitions();
      frameLayers.forEach(clearPhotoAnimation);
      bgLayers.forEach(clearPhotoAnimation);
    ...
    if (step.type === "chapter-title") {
      ...
      // Se precarga el capítulo completo mientras aparece su cartela.
      ...
      // El cambio forzado es realmente instantáneo: no se conserva ninguna
      // animación o transición CSS de la escena anterior.
      if (instant) {
        [currentFrame, standbyFrame, currentBg, standbyBg].forEach(function (layer) {
          clearPhotoAnimation(layer);
        });
      }
      ...
      standbyFrame.style.transition = instant ? "none" : "";
      void standbyFrame.offsetWidth;

      if (instant) {
        // La foto principal cambia directamente, sin transición.
        standbyFrame.classList.add("is-top");
      } else {
        // Cada reproducción automática obtiene una transición nueva.
        var transition = getRandomTransition();
        standbyFrame.classList.add("is-top", transition.enter);
      }

      currentFrame.classList.remove("is-top");
      frameTopIndex = 1 - frameTopIndex;

      // El fondo es siempre la misma foto, pero desenfocada por CSS.
      // En automático se mantiene el fondo anterior durante toda la entrada
      // de la foto y un pequeño retraso adicional; después hace crossfade.
      standbyBg.style.backgroundImage = 'url("' + step.src + '")';
      standbyBg.className = "photo-bg";
      standbyBg.style.animation = "";
      standbyBg.style.transition = instant ? "none" : "";
      ...
      if (instant) {
        ...
        restorePhotoAnimationStyles();
      } else {
        // La duración se obtiene de la animación seleccionada.
        // El fondo espera a que termine la entrada + 350 ms.
        var bgDelay = (transition.duration || 1200) + 350;
        backgroundTimer = setTimeout(function () {
          backgroundTimer = null;
          if (token !== transitionToken) return;

          standbyBg.classList.add("is-top", "t-bg-crossfade");
          currentBg.classList.remove("is-top");
          bgTopIndex = 1 - bgTopIndex;

          setTimeout(function () {
            if (token !== transitionToken) return;
            standbyBg.classList.remove("t-bg-crossfade");
          }, 1200);
        }, bgDelay);
      }
