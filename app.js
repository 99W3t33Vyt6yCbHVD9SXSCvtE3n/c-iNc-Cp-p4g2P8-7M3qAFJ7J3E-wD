...
(function () {
  ...
  function renderStep(instant) {
    ...
    if (step.type === "chapter-title") {
      ...
      // Ajuste temporal v12: reducimos aproximadamente 0,5 s por foto
      // (unos 5 s cada 10 fotos), repartiendo el recorte entre la duración
      // de la transición y el tiempo de permanencia estable. Las transiciones
      // más largas absorben una parte ligeramente mayor del recorte.
      var baseHoldDuration = step.isLastPhoto ? lastPhotoDurationMs : photoDurationMs;
      var targetReductionMs = 500;
      var transitionDuration = 0;
      var holdDuration = baseHoldDuration;
      if (transition) {
        var totalVisualTime = transition.duration + baseHoldDuration;
        var transitionReduction = targetReductionMs * (transition.duration / totalVisualTime);
        var holdReduction = targetReductionMs * (baseHoldDuration / totalVisualTime);
        transitionDuration = Math.max(1000, Math.round(transition.duration - transitionReduction));
        holdDuration = Math.max(1000, Math.round(baseHoldDuration - holdReduction));
      }
      ...
      preloadUpcoming(stepIndex + 1, 2);
      // En v12, holdDuration ya contiene el tiempo ajustado de permanencia
      // completamente visible después de terminar la transición. La suma de
      // transición + permanencia se ha reducido en aproximadamente 500 ms
      // respecto a la v11 para cada foto.
